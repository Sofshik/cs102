...
def repo_create(workdir: tp.Union[str, pathlib.Path]) -> pathlib.Path:
    ...
    HEAD = "HEAD"
    with open(workdir / gitdir_name / HEAD, "w") as i:
        i.write("ref: refs/heads/master\n")
    config = "config"
    with open(workdir / gitdir_name / config, "w") as i:
        i.write(
            "[core]\n\trepositoryformatversion = 0\n\tfilemode = true\n\tbare = false\n\tlogallrefupdates = false\n"
        )
    description = "description"
    with open(workdir / gitdir_name / description, "w") as i:
        i.write("Unnamed pyvcs repository.\n")
    objects = "objects"
